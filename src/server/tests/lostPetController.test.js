import { submitForm } from "../controllers/lostPetController";
...
describe("Lost Pet Controller Test", () => {
    describe("Submit Form", () => {
        ...
        it("should return 200, and have added form to firestore", async () => {
            getCoordinateFromLocation.mockResolvedValue({
                lat: -37.833401,
                lng: 144.980331,
                state: "VIC",
            });
            ...
            expect(getCoordinateFromLocation).toHaveBeenCalledWith(
                "Test Location"
            );
        ...
        it("should throw 400 error if unable to get coordinates from address", async () => {
            getCoordinateFromLocation.mockRejectedValue(
                new Error("Unable to get coordinates for the location")
            );
            ...
            expect(res.status).toHaveBeenCalledWith(400);
            expect(res.send).toHaveBeenCalledWith({
                message: "Unable to get coordinates for the location",
                ok: false,
            });
        });
